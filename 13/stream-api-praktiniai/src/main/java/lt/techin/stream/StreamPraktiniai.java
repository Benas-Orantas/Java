...
public class StreamPraktiniai {
    ...
    public static double getAverageAge(List<User> users){
        //return users.stream().map(u -> u.getAge()).reduce(0, (a, b) -> a + b) / users.size();
    ...
    public static boolean anyMatch(List<User> users, int age){
        return users.stream().mapToInt(User::getAge).anyMatch(u -> u == age);
    }
...
//        return users.stream().sorted((u1, u2) -> u1.getAge().compareTo(u2.getAge())).collect(Collectors.toList());
    ...
    public static List<Integer> generateFirst10PrimeNumbers(){
        return IntStream.rangeClosed(2, 29).filter(i -> isPrime(i)).boxed().collect(Collectors.toList());
    }
    ...
    public static List<Integer> generate10RandomNumbers(){
        return IntStream.generate(() -> (int) (Math.random() * 10)).boxed().limit(10).toList();
    }
...
//        throw new UnsupportedOperationException("Not implemented");
    ...
    public static Map<Boolean, Long> countGender(List<User> users){
        return users.stream().collect(Collectors.partitioningBy(u -> u.isMale(), Collectors.counting()));
    }
